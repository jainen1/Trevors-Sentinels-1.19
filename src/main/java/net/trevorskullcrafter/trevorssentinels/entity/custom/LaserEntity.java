...
public class LaserEntity extends PersistentProjectileEntity {
    ...
    @Override public void tick() {
        super.tick(); if(getWorld().isClient()) for(int i = 0; i <= 4; i++) getWorld().addParticle(
    ...
    @Override public boolean collidesWithStateAtPos(BlockPos pos, BlockState state) {
        return !(projectileType==1 && state.isIn(BlockTagGenerator.LASER_PASS));
    }

    private void reflectLaser1(Direction sideHit) {
        LaserEntity laser = new LaserEntity(getWorld(), this.getX(), this.getY(), this.getZ(), maxLifeTime-1, laserSpeed-0.2f,
        ...
        if(getOwner()!=null) laser.setOwner(getOwner()); getWorld().spawnEntity(laser);
    }
        ...
        if(hitResult.getType() != HitResult.Type.MISS && getWorld() instanceof ServerWorld serverWorld) {
            if (type == HitResult.Type.ENTITY && ((EntityHitResult) hitResult).getEntity() instanceof LivingEntity entity) {
                if (damageAmount < 1) {
                    ...
                    entity.damage(entity.getDamageSources().mobProjectile(this, getOwner() instanceof LivingEntity livingEntity? livingEntity : null), damageAmount);
                ...
                if(collidesWithStateAtPos(blockHitResult.getBlockPos(), state)){
                    ...
                    if(projectileType != 2) doExplosion(serverWorld);
                    if(projectileType == 1 && state.isIn(BlockTagGenerator.LASER_REFLECTIVE)) reflectLaser1(blockHitResult.getSide());

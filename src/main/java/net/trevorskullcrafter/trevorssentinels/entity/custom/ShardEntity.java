...
public class ShardEntity extends ThrownItemEntity {
    ...
    public ShardEntity(World world, LivingEntity owner) { super(ModEntities.GALLIUM_SHARD, owner, world); }
    public ShardEntity(World world, double x, double y, double z) { super(ModEntities.GALLIUM_SHARD, x, y, z, world); }

    @Override protected Item getDefaultItem() { return ModItems.SCRAP_METAL_SHARD; }
    ...
    @Environment(EnvType.CLIENT) private ParticleEffect getParticleParameters() { // Not entirely sure, but probably has to do with the snowball's particles. (OPTIONAL)
        ItemStack itemStack = ModItems.SCRAP_METAL_SHARD.getDefaultStack();

...
public class HoloprojectorItem extends Item implements StyleUtil.StyleSwitcher {
                ...
                switch (StyleUtil.getStyle(itemStack)){
                    case 1 -> {
                        if(itemStack.getDamage() < itemStack.getMaxDamage()-1) {
                            if(world.canPlace(ModBlocks.HOLOBARRIER.getPlacementState(new ItemPlacementContext(context)), blockPos.offset(context.getSide()), ShapeContext.absent())){
                                context.getWorld().setBlockState(blockPos.offset(context.getSide()), ModBlocks.HOLOBARRIER.getPlacementState(new ItemPlacementContext(context)), 11);

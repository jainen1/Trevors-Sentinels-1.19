...
public class LilithBladeItem extends SwordItem {
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        ...
        if (!world.isClient() && hand == Hand.MAIN_HAND) {
            if (Screen.hasControlDown()) {
                ...
                if(itemStack.getSubNbt("trevorssentinels:raptureMode").getInt("trevorssentinels:raptureModeInt") == 2){
    ...
    public boolean postHit(ItemStack itemStack, LivingEntity target, LivingEntity attacker) {
        if(itemStack.getSubNbt("trevorssentinels:raptureMode").getInt("trevorssentinels:raptureModeInt") == 1){
            ...
            target.addStatusEffect(new StatusEffectInstance(StatusEffects.INSTANT_DAMAGE,1,3,false,false,false));
        }
        return true;
    }

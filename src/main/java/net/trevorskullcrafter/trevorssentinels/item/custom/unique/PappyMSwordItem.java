    ...
    @Override public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand){
        if(!world.isClient()) {
            if (StyleUtil.getStyle(user.getMainHandStack()) == 1) {
                ...
                world.playSoundFromEntity(null, user, SoundEvents.ENTITY_WARDEN_SONIC_CHARGE, SoundCategory.BLOCKS, 2.0F, 5.0F);
                ...
                world.playSoundFromEntity(null, user, SoundEvents.BLOCK_ANVIL_LAND, SoundCategory.BLOCKS, 2.0F, Random.createLocal().nextFloat());
    ...
    @Override public void inventoryTick(ItemStack stack, World world, Entity entity, int slot, boolean selected) {
        if(!world.isClient() && selected && entity instanceof PlayerEntity player) {
            ...
            if (StyleUtil.getStyle(stack) == 2 && getCharge(stack) == 6) {
                ...
                player.sendMessage(Text.literal("◆ ◆ ◆ ◆ ◆ ◆").formatted(world.getTime() % 2 == 0 ? Formatting.AQUA : Formatting.DARK_AQUA), true);
            }

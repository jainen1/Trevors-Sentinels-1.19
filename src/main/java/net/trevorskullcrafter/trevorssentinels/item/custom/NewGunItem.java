...
public class NewGunItem extends Item implements GeoItem, ToolSkinnable, StyleUtil.StyleSwitcher, Reloadable {
    ...
    Text[] tooltipData; Item fuelItem; StatusEffectInstance[] effects;

    public NewGunItem(Settings settings, StatusEffectInstance... effects) {
    ...
    @Override public void appendTooltip(ItemStack stack, @Nullable World world, List<Text> tooltip, TooltipContext context) {
        //initialization
        if(tooltipData == null) {
            tooltipData = new Text[5];
            float damage = getDamage(stack); int count = getCount(stack); float speed = getSpeed(stack); int cooldown = getShotCooldown(stack);
            tooltipData[0] = Text.literal(getGunclass(stack).getName());
            tooltipData[1] = Text.literal(" / " + getMaxAmmo(stack) +" ⚡ ").formatted(Formatting.GRAY);
            tooltipData[2] = Text.literal(count + " ☄").formatted(TextUtil.quotientToolTipFormatting(count, 8));
            double firingRate = Math.floor(20.0/(cooldown+1) * 10) / 10;
            if(firingRate <= 1) { tooltipData[3] = Text.literal(new DecimalFormat("#.##").format(speed)+" →")
                    .formatted(TextUtil.quotientToolTipFormatting(speed, 4)); }
            else { tooltipData[3] = Text.literal(firingRate+" \uD83D\uDD25").formatted(TextUtil.quotientToolTipFormatting(firingRate, 5)); }
            tooltipData[4] = Text.literal(Math.abs(damage)+(damage < 1 ? " ✚" : " ☠"))
                    .formatted(TextUtil.quotientToolTipFormatting(Math.abs(damage), 20));
        }
        //stats
        tooltip.add(tooltipData[0]);
        ...
        int shotCooldown = getShotCooldown(stack);
        int reloadCooldown = getReloadCooldown(stack);

        Text[] modifierTooltips = new Text[10];
        modifierTooltips[0] = Text.literal("☄ " + count).formatted(TextUtil.quotientToolTipFormatting(count, 8)); //count
        modifierTooltips[1] = TextUtil.coloredText("\uD83C\uDF08 #" + Integer.toHexString(color.getRGB()).substring(2), color); //color
        modifierTooltips[2] = Text.literal((damage < 1 ? "✚ " : "☠ ") + damage).formatted(TextUtil.quotientToolTipFormatting(damage, 20)); //damage
        modifierTooltips[3] = Text.literal("t " + maxAmmo).formatted(TextUtil.quotientToolTipFormatting(maxAmmo, 20)); //maxammo
        modifierTooltips[4] = Text.literal("→ " + speed).formatted(TextUtil.quotientToolTipFormatting(speed, 4)); //speed
        modifierTooltips[5] = Text.literal("t " + divergence).formatted(TextUtil.quotientToolTipFormatting(divergence, 4)); //divergence
        modifierTooltips[6] = Text.literal("t " + recoil).formatted(TextUtil.quotientToolTipFormatting(recoil, 4)); //recoil
        modifierTooltips[7] = Text.literal("t " + maxAge);//.formatted(TextUtil.quotientToolTipFormatting(maxAge, 4)); //maxage
        modifierTooltips[8] = Text.literal("t " + shotCooldown).formatted(TextUtil.quotientToolTipFormatting(shotCooldown, 4)); //shotcooldown
        modifierTooltips[9] = Text.literal("t " + reloadCooldown);//.formatted(TextUtil.quotientToolTipFormatting(reloadCooldown, 4)); //reloadcooldown

        tooltip.addAll(Arrays.asList(modifierTooltips));

        Text column = Text.literal(" | ").formatted(Formatting.DARK_GRAY);
        tooltip.add(Text.empty().append(tooltipData[2]).append(column).append(tooltipData[3]).append(column).append(tooltipData[4]));
        //effect

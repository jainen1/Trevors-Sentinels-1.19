...
public class ThanatuBladeItem extends SwordItem {
        ...
        if (!world.isClient && hand == Hand.MAIN_HAND) {
            if (Screen.hasControlDown()) {
                if (Objects.requireNonNull(itemStack.getSubNbt("trevorssentinels:raptureMode")).getInt("trevorssentinels:raptureModeInt") == 1) {
                    ...
                    user.sendMessage(Text.empty().append(trevorssentinels.styleText).append(Text.literal("Riftcaller").formatted(Formatting.DARK_PURPLE)), true);
                    ...
                    user.sendMessage(Text.empty().append(trevorssentinels.styleText).append(Text.literal("Riftwalker").formatted(Formatting.DARK_PURPLE)), true);
                ...
                if (itemStack.getSubNbt("trevorssentinels:raptureMode").getInt("trevorssentinels:raptureModeInt") == 1) {
                    if (Screen.hasShiftDown()) {
                        ...
                        timePercent = (int) (itemStack.getSubNbt("trevorssentinels:timerHolder").getLong("trevorssentinels:timer") - world.getTime()) / 2;
    ...
    @Override public void inventoryTick(ItemStack itemStack, World world, Entity entity, int slot, boolean selected) {
        if(!world.isClient) {
            if (itemStack.getSubNbt("trevorssentinels:harmoniousTeleportData") != null) {
                if (Objects.requireNonNull(itemStack.getSubNbt("trevorssentinels:timerHolder")).getLong("trevorssentinels:timer") <= world.getTime()) {
    ...
    @Override public void appendTooltip(ItemStack itemStack, @Nullable World world, List<Text> tooltip, TooltipContext context) {
        if(itemStack.getSubNbt("trevorssentinels:raptureMode") != null) {
            if (Objects.requireNonNull(itemStack.getSubNbt("trevorssentinels:raptureMode")).getInt("trevorssentinels:raptureModeInt") == 1) {
                tooltip.add(Text.empty().append(trevorssentinels.styleText).append(Text.literal("Riftwalker").formatted(Formatting.DARK_PURPLE)));
                ...
                tooltip.add(Text.empty().append(trevorssentinels.styleText).append(Text.literal("Riftcaller").formatted(Formatting.DARK_PURPLE)));
